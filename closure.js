...
function saveOutput(func, magicWord) {
  const output = {};
  return function almostSameFunc(input) {
    if (input === magicWord) {
      return output;
    }
...
function censor() {
  ...
  return function(...strings) {
    if (strings.length === 2) {
      return output[strings[0]] = strings[1];
    }
...
function createSecretHolder(secret) {
  let safeSecret = secret;
  function getSecret() {
    return safeSecret;
  }
  function setSecret(newSecret) {
    safeSecret = newSecret;
  }
...
// CHALLENGE 16
function average() {
  let count = 0, total = 0, average = 0;
  return function(num) {
    if (num === undefined && total === 0) {
      return 0;
    }
    if (num === undefined) {
      return average;
